...
# kubectl -n kubermatic get deploy kubermatic-operator -o jsonpath='{.spec.replicas}' && echo
...
# IMPORTANAT NOTE: You must run the user-cluster-controller-manager locally (instead of using the one built into the operator) in order to test the new headlamp deployment logic. This is because the operator's controller manager will still have the old code which doesn't know how to deploy headlamp, so it will keep trying to deploy kubernetes-dashboard instead and overwrite our changes. By running the user-cluster-controller-manager locally, we can test the new code in isolation without interference from the operator.
# Set namespace context first
kubectl config set-context --current --namespace=${CLUSTER_NS}


# Troubleshooting tip: If you see errors about the kubeconfig or permissions, make sure your current kubectl context is set to the correct cluster namespace (cluster-iq896dvgx8) and that you have the necessary permissions to access the cluster. You can check your current context with `kubectl config current-context` and switch contexts with `kubectl config use-context <context-name>`.

# kubectl config get-contexts $(kubectl config current-context)
# CURRENT   NAME         CLUSTER      AUTHINFO   NAMESPACE
# *         fth4x68gcq   fth4x68gcq   default    cluster-iq896dvgx8
